...
void Mader::clearTrajs() {
    mtx_trajs_.lock();
    trajs_.clear();
    mtx_trajs_.unlock();
}

std::vector<Mader::VectorDIM> Mader::getPlan() {
    std::vector<Mader::VectorDIM> plan;
    for (auto& p : plan_) {
...
Mader::Mader(mt::parameters par, double max_runtime)
    : par_(par), max_runtime_(max_runtime) {
    std::cout << "initialized max run time: " << max_runtime_ << std::endl;
    ...
    // std::cout << "seperator solver constructed." << std::endl;
}

void Mader::dynTraj2dynTrajCompiled(const mt::dynTraj& traj,
...
void Mader::updateTrajObstacles(mt::dynTraj traj, double current_timestamp) {
    ...
    for (int index_traj = 0; index_traj < trajs_.size(); index_traj++) {
        ...
        mtx_t_.lock();
        t_ = current_timestamp;

        Eigen::Vector3d center_obs =
            trajs_[index_traj].traj->eval(0.0, 0).value();
...
std::vector<Eigen::Vector3d> Mader::vertexesOfInterval(
    ...
    for (int i = index_first_interval; i <= index_last_interval; i++) {
        ...
        for (int j = 0; j < V.cols(); j++) {
            ...
            if (delta.norm() < 1e-6) {  // no inflation
                ...
                // // x,y,z
                points.push_back(Eigen::Vector3d(x + delta.x(), y + delta.y(),
                                                 z + delta.z()));
                points.push_back(Eigen::Vector3d(x + delta.x(), y - delta.y(),
                                                 z - delta.z()));
                points.push_back(Eigen::Vector3d(x + delta.x(), y + delta.y(),
...
std::vector<Eigen::Vector3d> Mader::vertexesOfInterval(
    ...
    if (traj.is_agent == false) {
        ...
        for (double t = t_start;  /////////////
            ...
            VectorDIM position =
                traj.traj
                    ->eval(std::max(0.0, t_ - current_timestamp_ - 1e-10), 0)
                    .value();
...
CGAL_Polyhedron_3 Mader::convexHullOfInterval(mt::dynTrajCompiled& traj,
...
void Mader::removeTrajsThatWillNotAffectMe(const mt::state& A, double t_start,
                                           double t_end) {
    std::vector<int> ids_to_remove;
    ...
    for (auto traj : trajs_) {
        ...
        if (traj.is_static == true) {
            ...
            if (traj.is_agent == false) {
                center_obs =
                    traj.traj
                        ->eval(std::max(0.0, t_ - current_timestamp_ - 1e-10),
                               0)
                        .value();
            } else {
...
ConvexHullsOfCurve Mader::convexHullsOfCurve(mt::dynTrajCompiled& traj,
                                             double t_start, double t_end) {
...
void Mader::getG(mt::state& G) { G = G_; }
...
bool Mader::trajsAndPwpAreInCollision(mt::dynTrajCompiled traj,
...
bool Mader::replan(mt::Edges& edges_obstacles_out,
    ...
    if (isReplanningNeeded() == false) {
        std::cout << "Mader: Replanning not needed" << std::endl;
        return false;
    }
    std::cout << "Mader: Replanning needed" << std::endl;
    ...
    mu::saturate(deltaT_, par_.lower_bound_runtime_snlopt / par_.dc,
                 par_.upper_bound_runtime_snlopt / par_.dc);
    ...
    // std::cout << blue << "plan_.size():" << plan_.size() << reset <<
    // std::endl;
    ...
    if (k_index_end != 0) {
        runtime_snlopt =
            k_index * par_.dc;  // std::min(, par_.upper_bound_runtime_snlopt);
    } else {
        runtime_snlopt =
            par_.upper_bound_runtime_snlopt;  // I'm stopped at the end of the
                                              // trajectory --> take my time to
                                              // replan
    }
    runtime_snlopt = max_runtime_;
    mu::saturate(runtime_snlopt, par_.lower_bound_runtime_snlopt,
                 par_.upper_bound_runtime_snlopt);
    std::cout << "runtime_snlopt: " << runtime_snlopt << std::endl;
    ...
    //////////////////////
    double time_now = current_timestamp;
    ...
    std::cout << on_cyan << bold << "Solved so far" << solutions_found_ << "/"
              << total_replannings_ << reset << std::endl;
    ...
    if (result == false) {
        int states_last_replan =
            ceil(replanCB_t.ElapsedMs() /
                 (par_.dc * 1000));  // Number of states that
    ...
    if ((plan_size - 1 - k_index_end) < 0) {
        ...
        plan_.erase(plan_.end() - k_index_end - 1,
    ...
    if (exists_previous_pwp_ == true) {
        pwp_out =
            mu::composePieceWisePol(time_now, par_.dc, pwp_prev_, pwp_now);
        pwp_prev_ = pwp_out;
    } else {  //
        pwp_out = pwp_now;
        pwp_prev_ = pwp_now;
    ...
    mt::state F = plan_.back();  // Final point of the safe path (\equiv final
                                 // point of the comitted path)
    ...
    int states_last_replan = ceil(replanCB_t.ElapsedMs() /
                                  (par_.dc * 1000));  // Number of states that
...
void Mader::getDesiredYaw(mt::state& next_goal) {
    ...
    switch (drone_status_) {
        case DroneStatus::YAWING:
            desired_yaw = atan2(G_term_.pos[1] - next_goal.pos[1],
                                G_term_.pos[0] - next_goal.pos[0]);
            diff = desired_yaw - state_.yaw;
            break;
        case DroneStatus::TRAVELING:
        case DroneStatus::GOAL_SEEN:
            desired_yaw = atan2(M_.pos[1] - next_goal.pos[1],
                                M_.pos[0] - next_goal.pos[0]);
            diff = desired_yaw - state_.yaw;
            break;
...
bool Mader::peekNextGoal(mt::state& next_goal) {
...
bool Mader::getNextGoal(mt::state& next_goal) {
    ...
    std::cout << "plan consumed. plan_.size(): " << plan_.size() << std::endl;
    getDesiredYaw(next_goal);
