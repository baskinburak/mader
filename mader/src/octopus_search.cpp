...
OctopusSearch::OctopusSearch(std::string basis, int num_pol, int deg_pol,
                             double alpha_shrink) {
    p_ = deg_pol;
    M_ = num_pol + 2 * p_;
    N_ = M_ - p_ - 1;
    ...
    if (basis == "MINVO") {
        // std::cout << green << bold << "A* is using MINVO" << reset <<
        // std::endl;
        M_pos_bs2basis_ = basis_converter.getMinvoPosConverters(num_pol);
        M_vel_bs2basis_ = basis_converter.getBSplineVelConverters(
            num_pol);  // getMinvoVelConverters TODO!!
        basis_ = MINVO;
    } else if (basis == "BEZIER") {
        // std::cout << green << bold << "A* is using BEZIER" << reset <<
        // std::endl;
        M_pos_bs2basis_ = basis_converter.getBezierPosConverters(num_pol);
        M_vel_bs2basis_ = basis_converter.getBSplineVelConverters(
            num_pol);  // getBezierVelConverters TODO!!
        basis_ = BEZIER;
    } else if (basis == "B_SPLINE") {
        // std::cout << green << bold << "A* is using B_SPLINE" << reset <<
        // std::endl;
...
void OctopusSearch::setUp(double t_min, double t_max,
                          const mt::ConvexHullsOfCurves_Std& hulls) {
    ...
    for (int i = (p_ + 1); i <= M_ - p_ - 1; i++) {
        knots[i] =
            knots[i - 1] + deltaT;  // Assumming a uniform b-spline (internal
...
void OctopusSearch::setVisual(bool visual) { visual_ = visual; }

void OctopusSearch::getBestTrajFound(mt::trajectory& best_traj_found,
                                     mt::PieceWisePol& pwp, double dc) {
    mt::trajectory traj;
...
void OctopusSearch::getEdgesConvexHulls(mt::Edges& edges_convex_hulls) {
    ...
    for (int i = 3; i < result_.size(); i++) {
        ...
        for (int j = 0; j < 4; j++) {  // For every point in the convex hull
            mt::Edge edge;
            edge.first = last4Cps_new_basis.col(j);
            for (int i = 0; i < 4; i++) {  // generate an edge from that point j
                                           // to the other points i!=j
                if (i == j) {
                    continue;
                } else {
...
void OctopusSearch::getAllTrajsFound(
    ...
    for (auto node : expanded_valid_nodes_) {
        ...
        while (tmp != NULL) {
            cps.push_back(
                Eigen::Vector3d(tmp->qi.x(), tmp->qi.y(), tmp->qi.z()));
            tmp = tmp->previous;
        }
        ...
        std::reverse(std::begin(cps),
                     std::end(cps));  // cps=[q0 q1 q2 q3 q4 ...]

        mt::trajectory traj;
        mt::PieceWisePol pwp;
        CPs2TrajAndPwp(cps, traj, pwp, N_, p_, num_pol_, knots_,
                       0.01);  // Last number is the resolution
...
void OctopusSearch::setMaxValuesAndSamples(Eigen::Vector3d& v_max,
    ...
    num_samples_x_ =
        (num_samples_x % 2 == 0) ? ceil(num_samples_x) : num_samples_x;
    num_samples_y_ =
        (num_samples_y % 2 == 0) ? ceil(num_samples_y) : num_samples_y;
    num_samples_z_ =
        (num_samples_z % 2 == 0) ? ceil(num_samples_z) : num_samples_z;
    ...
    for (int jx : indexes_samples_x_) {
        for (int jy : indexes_samples_y_) {
            for (int jz : indexes_samples_z_) {
                // std::cout << "Pushing combination " << jx << ", " << jy << ",
                // " << jz << std::endl;
                all_combinations_.push_back(
                    std::tuple<int, int, int>(jx, jy, jz));
            }
        }
    }

    unsigned seed = std::chrono::system_clock::now().time_since_epoch().count();
    shuffle(all_combinations_.begin(), all_combinations_.end(),
    ...
    voxel_size_ = min_voxel_size +
                  fraction_voxel_size * (max_voxel_size - min_voxel_size);

    orig_ = q2_ - Eigen::Vector3d(bbox_x_ / 2.0, bbox_y_ / 2.0, bbox_z_ / 2.0);
}

void OctopusSearch::setXYZMinMaxAndRa(double x_min, double x_max, double y_min,
...
void OctopusSearch::setBias(double bias) { bias_ = bias; }

void OctopusSearch::setGoal(Eigen::Vector3d& goal) { goal_ = goal; }

void OctopusSearch::setRunTime(double max_runtime) {
    max_runtime_ = max_runtime;
}
void OctopusSearch::setGoalSize(double goal_size) { goal_size_ = goal_size; }

// returns the minimum voxel_size needed (if bigger, all the neighbous from q2_
// are inside the voxel of q2_) min_voxel_size: if voxel_size < min_voxel_size,
// all the neighbous from q2_ will be expanded correctly max_voxel_size: if
// voxel_size > max_voxel_size, there will be no nodes expanded from q2_ (they
// are on the same cell as q2_)

void OctopusSearch::computeLimitsVoxelSize(double& min_voxel_size,
                                           double& max_voxel_size) {
    int i = 2;
    double constraint_xL, constraint_xU, constraint_yL, constraint_yU,
        constraint_zL, constraint_zU;

    computeUpperAndLowerConstraints(i, q0_, q1_, q2_, constraint_xL,
                                    constraint_xU, constraint_yL, constraint_yU,
                                    constraint_zL, constraint_zU);
    ...
    for (int jx : indexes_samples_x_) {
        for (int jy : indexes_samples_y_) {
            for (int jz : indexes_samples_z_) {
                // sample a velocity
                Eigen::Vector3d vi;
                vi << constraint_xL + jx * ((constraint_xU - constraint_xL) /
                                            (num_samples_x_ - 1)),  /////////
                    constraint_yL + jy * ((constraint_yU - constraint_yL) /
                                          (num_samples_y_ - 1)),  /////////
                    constraint_zL + jz * ((constraint_zU - constraint_zL) /
                                          (num_samples_z_ - 1));  /////////

                if (vi.norm() <
                    0.000001)  // remove the cases where vi is very small
                {
                    continue;
                }

                //  std::cout << "vx= " << vi.x() << ", vy= " << vi.y() << ",
                //  vz= " << vi.z() << std::endl; std::cout << "(neighbor_of_q2
                //  - q2_).norm()= " << (neighbor_of_q2 - q2_).norm() <<
                //  std::endl;

                neighbor_of_q2 =
                    (knots_(i + p_ + 1) - knots_(i + 1)) * vi / (1.0 * p_) +
                    q2_;

                min_voxel_size =
                    std::min(min_voxel_size, (neighbor_of_q2 - q2_).norm());
                max_voxel_size =
                    std::max(max_voxel_size, (neighbor_of_q2 - q2_).norm());
            }
        }
    }
}
...
// // axis=0 (x), 1(y) or 2(z)
// bool OctopusSearch::computeAxisForNextInterval(const int i, const
// Eigen::Vector3d& viM1, int axis, double& constraint_L,
//                                                double& constraint_U)
// {
...
//     {
//       double g1 = viM1(axis);
//       double tmp = -(M_interv_next(1, j) / M_interv_next(2, j)) +
//       (M_interv_next(1, i) / M_interv_next(2, i)); double qli =
//       (-mu::sgn(M_interv_next(2, i)) * v_max_.x() - g1 * M_interv_next(0, i))
//       / M_interv_next(2, i); double quj = (mu::sgn(M_interv_next(2, j)) *
//       v_max_.x() - g1 * M_interv_next(0, j)) / M_interv_next(2, j); double
//       bound = (qli - quj) / tmp; if (tmp >= 0)
//       {
...
// Compute the lower and upper bounds on the velocity based on the velocity and
// acceleration constraints return false if any of the intervals, the lower
// bound is > the upper bound
bool OctopusSearch::computeUpperAndLowerConstraints(
    const int i, const Eigen::Vector3d& qiM2, const Eigen::Vector3d& qiM1,
    ...
    int interv =
        i - 2;  // index of the interval (i is the index of the control point)

    Eigen::Vector3d viM2 = p_ * (qiM1 - qiM2) /
                           (knots_(i - 2 + p_ + 1) -
                            knots_(i - 2 + 1));  // velocity_{current.index -2}
    Eigen::Vector3d viM1 = p_ * (qi - qiM1) /
                           (knots_(i - 1 + p_ + 1) -
                            knots_(i - 1 + 1));  // velocity_{current.index -1}
    ...
    if (i == (N_ - 3)) {
        ...
        {
            if (isNotZero(M_interv(
                    1, j)))  // If it's zero, no need to impose this constraint
            {
                double vi_bound1 = (v_max_.x() - tmp2(0, j)) / M_interv(1, j);
                double vi_bound2 = (-v_max_.x() - tmp2(0, j)) / M_interv(1, j);
                constraint_xL =
                    std::max(constraint_xL, std::min(vi_bound1, vi_bound2));
                constraint_xU =
                    std::min(constraint_xU, std::max(vi_bound1, vi_bound2));
            }
        }
        ...
        for (int j = 0; j < 3; j++)  // For the three velocity control points
        {
                    ...
                    1, j)))  // If it's zero, no need to impose this constraint
            {
                ...
                constraint_yL =
                    std::max(constraint_yL, std::min(vi_bound1, vi_bound2));
                constraint_yU =
                    std::min(constraint_yU, std::max(vi_bound1, vi_bound2));
            }
        }
        ...
        for (int j = 0; j < 3; j++)  // For the three velocity control points
        {
                    ...
                    1, j)))  // If it's zero, no need to impose this constraint
            {
                ...
                constraint_zL =
                    std::max(constraint_zL, std::min(vi_bound1, vi_bound2));
                constraint_zU =
                    std::min(constraint_zU, std::max(vi_bound1, vi_bound2));
            }
        }
        ...
        {
            if (isNotZero(M_interv(
                    0, j)))  // If it's zero, no need to impose this constraint
            {
    ...
    Eigen::Vector3d max_vel =
        a_max_ * d + viM1;  // this is to ensure that aiM1 is inside the bounds
    Eigen::Vector3d min_vel =
        -a_max_ * d + viM1;  // this is to ensure that aiM1 is inside the bounds
    ...
    if (i == (N_ - 3)) {  // impose also that aNm3 is satisfied
    ...
    auto areVeryClose = [&](double a, double b) {
        return (fabs(a - b) < eps_delta);
    };

    if (constraint_xL > constraint_xU || constraint_yL > constraint_yU ||
        constraint_zL > constraint_zU ||
        ...
        // std::cout << red << "x: " << constraint_xL << " --> " <<
        // constraint_xU << " || "
        //           << "y: " << constraint_yL << " --> " << constraint_yU << "
        //           || "
        //           << "z: " << constraint_zL << " --> " << constraint_zU <<
        //           reset << std::endl;
        // std::cout << "Interval is zero" << std::endl;
        return false;
    } else {
        // std::cout << green << "x: " << constraint_xL << " --> " <<
        // constraint_xU << " || "
        //           << "y: " << constraint_yL << " --> " << constraint_yU << "
        //           || "
        //           << "z: " << constraint_zL << " --> " << constraint_zU <<
    ...
    //   Eigen::Vector3d aNm2 = (p_ - 1) * (vNm1 - vNm2) / (knots_(N_ - 2 + p_ +
    //   1) - knots_(N_ - 2 + 2));

    //   if ((aNm2.array() > a_max_.array()).any() || (aNm2.array() <
    //   -a_max_.array()).any())
    //   {
    //     continue;
    //   }
...
void OctopusSearch::computeInverses() {
    ...
    for (int dim = 3; dim < N_; dim++) {
        ...
        Eigen::Matrix<double, -1, 1> tmp1 =
            -8 * Eigen::MatrixXd::Ones(dim - 1, 1);
        Eigen::Matrix<double, -1, 1> tmp2 =
            2 * Eigen::MatrixXd::Ones(dim - 2, 1);
        A.diagonal(1) = tmp1;
        A.diagonal(-1) = tmp1;
        A.diagonal(2) = tmp2;
        A.diagonal(-2) = tmp2;
...
void OctopusSearch::setq0q1q2(Eigen::Vector3d& q0, Eigen::Vector3d& q1,
                              Eigen::Vector3d& q2) {
    q0_ = q0;
    q1_ = q1;
    q2_ = q2;
...
double OctopusSearch::h(Node& node) { return (node.qi - goal_).norm(); }
...
double OctopusSearch::weightEdge(Node& node1,
                                 Node& node2)  // edge cost when adding node 2
{
...
void OctopusSearch::recoverPath(Node* result_ptr) {
    ...
    std::reverse(std::begin(result_),
                 std::end(result_));  // result_ is [q0 q1 q2 q3 ...]
}

Eigen::Matrix<double, 3, 4> OctopusSearch::transformBSpline2otherBasis(
    const Eigen::Matrix<double, 3, 4>& Qbs, int interval) {
    return Qbs * M_pos_bs2basis_[interval];
}

Eigen::Matrix<double, 3, 4> OctopusSearch::transformOtherBasis2BSpline(
    const Eigen::Matrix<double, 3, 4>& Qmv, int interval) {
    return Qmv * M_pos_bs2basis_inverse_[interval];
}

bool OctopusSearch::checkFeasAndFillND(std::vector<Eigen::Vector3d>& q,
    ...
    Eigen::Matrix<double, 3, 4>
        last4Cps;  // Each column contains a control point
    ...
    for (int index_interv = 0; index_interv < (q.size() - 3); index_interv++) {
        ...
        Eigen::Matrix<double, 3, 4> last4Cps_new_basis =
            transformBSpline2otherBasis(last4Cps, index_interv);

        for (int obst_index = 0; obst_index < num_of_obst_; obst_index++) {
            Eigen::Vector3d n_i;
            double d_i;

            bool solved = separator_solver_->solveModel(
                n_i, d_i, hulls_[obst_index][index_interv], last4Cps_new_basis);

            // std::cout << "index_interv= " << index_interv << std::endl;
            if (solved == false) {
                ////////////////////////////// For debugging
                // std::cout << red << "\nThis does NOT satisfy the LP:
                // obstacle= " << obst_index
                //          << ", last index=" << index_interv + 3 << reset <<
                //          std::endl;
                ...
                // std::cout << bold << red << "[A*] The node provided doesn't
                // satisfy LPs" << reset << std::endl;
                ...
                ////////////////////////////// For debugging
                // std::cout << "\nThis satisfies the LP:  obstacle= " <<
                // obst_index << ", last index=" << index_interv + 3
                //           << std::endl;
            ...
            /*      std::cout << "solved with ni=" << n_i.transpose() <<
               std::endl; std::cout << "solved with d_i=" << d_i << std::endl;*/
    ...
    for (int i = 0; i <= (N_ - 3); i++) {
        ...
        vip1 = p_ * (q[i + 2] - q[i + 1]) /
               (knots_(i + 1 + p_ + 1) - knots_(i + 1 + 1));
        vip2 = p_ * (q[i + 3] - q[i + 2]) /
               (knots_(i + 1 + p_ + 1 + 1) - knots_(i + 1 + 1 + 1));

        if (i == (N_ - 2)) {
            std::cout << "knots_= " << knots_ << std::endl;
            std::cout << "knots_(i + 1 + 1 + 1)= " << knots_(i + 1 + 1 + 1)
                      << std::endl;
            std::cout << "knots_(i + 1 + p_ + 1 + 1)= "
                      << knots_(i + 1 + p_ + 1 + 1) << std::endl;
            std::cout << "i + 1 + 1 + 1= " << i + 1 + 1 + 1 << std::endl;
            std::cout << "i + 1 + p_ + 1 + 1= " << i + 1 + p_ + 1 + 1
                      << std::endl;
        }
        ...
        // if ((vi.array() > epsilon * v_max_.array()).any() || (vi.array() <
        // -epsilon * v_max_.array()).any())

        // Assumming here that all the elements of v_max_ are the same
        if (V_newbasis.maxCoeff() > epsilon * v_max_.x() ||
            V_newbasis.minCoeff() < -epsilon * v_max_.x()) {
            std::cout << red
                      << "velocity constraint for vi is not satisfied, i=" << i
                      << reset << std::endl;
        ...
        //   if ((vip1.array() > epsilon * v_max_.array()).any() ||
        //   (vip1.array() < -epsilon * v_max_.array()).any())
        //   {
        //     isFeasible = false;
        //     std::cout << red << "velocity constraint for vNm1 is not
        //     satisfied" << reset << std::endl;
        //   }
        // }
        if ((ai.array() > epsilon * a_max_.array()).any() ||
            (ai.array() < -epsilon * a_max_.array()).any()) {
            // std::cout << red << "acceleration constraints are not satisfied"
            // << reset << std::endl; std::cout << "ai= " << ai.transpose() <<
...
bool OctopusSearch::collidesWithObstacles(Node& current) {
    ...
    if (current.index >= 3) {
        ...
        bool collides =
            collidesWithObstaclesGivenVertexes(last4Cps, current.index);

        ////////
        if (current.index == (N_ - 2)) {
            // Check for the convex hull qNm4, qNm3, qNm2, qNm1 (where
            // qNm1==qNm2)

            Eigen::Matrix<double, 3, 4>
                last4Cps_tmp;  // Each column contains a control point
            ...
            collides = (collides || collidesWithObstaclesGivenVertexes(
                                        last4Cps_tmp, N_ - 1));

            // Check for the convex hull qNm3, qNm2, qNm1, qN (where
            // qN==qNm1==qNm2)
            ...
            collides = (collides ||
                        collidesWithObstaclesGivenVertexes(last4Cps_tmp, N_));
        }
        ////////

        return collides;
    } else {
        return false;  // I don't have enough control points yet to determine if
                       // the first interval collides or not
    }
}

void OctopusSearch::expandAndAddToQueue(Node& current, double constraint_xL,
                                        double constraint_xU,
                                        double constraint_yL,
                                        double constraint_yU,
                                        double constraint_zL,
                                        double constraint_zU) {
    ...
    for (auto comb : all_combinations_) {
        ...
        vi << constraint_xL + jx * delta_x, constraint_yL + jy * delta_y,
            constraint_zL + jz * delta_z;

        neighbor.qi =
            (knots_(i + p_ + 1) - knots_(i + 1)) * vi / (1.0 * p_) + current.qi;

        if (vi.norm() < 1e-5 ||  // Not wanna use v=[0,0,0]
            neighbor.qi.x() > x_max_ ||
            neighbor.qi.x() < x_min_ ||  /// Outside the limits
            neighbor.qi.y() > y_max_ ||
            neighbor.qi.y() < y_min_ ||  /// Outside the limits
            neighbor.qi.z() > z_max_ ||
            neighbor.qi.z() < z_min_ ||        /// Outside the limits
            (neighbor.qi - q0_).norm() >= Ra_  // ||  /// Outside the limits
            // (ix >= bbox_x_ / voxel_size_ ||                            //
            // Out. the search box
            //  iy >= bbox_y_ / voxel_size_ ||                            //
            //  Out. the search box iz >= bbox_z_ / voxel_size_) // Out. the
            //  search box
        )
        ...
        // std::cout << green << neighbor.qi.transpose() << " cost=" <<
        // neighbor.g + bias_ * neighbor.h << reset << std::endl;
    ...
    // std::cout << bold << "expanding took " << timer_expand << reset <<
    // std::endl;
...
bool OctopusSearch::collidesWithObstaclesGivenVertexes(
    const Eigen::Matrix<double, 3, 4>& last4Cps, int index_lastCP) {
    // MyTimer timer_function(true);
    // std::cout << "In collidesWithObstaclesGivenVertexes, index_lastCP= " <<
    // index_lastCP << std::endl;

    int interval = index_lastCP - 3;

    Eigen::Matrix<double, 3, 4> last4Cps_new_basis =
        transformBSpline2otherBasis(last4Cps, interval);
    ...
    for (int obst_index = 0; obst_index < num_of_obst_; obst_index++) {
        satisfies_LP = separator_solver_->solveModel(
            n_i, d_i, hulls_[obst_index][interval], last4Cps_new_basis);
...
bool OctopusSearch::run(std::vector<Eigen::Vector3d>& result,
                        std::vector<Eigen::Vector3d>& n,
                        std::vector<double>& d) {
    /////////// reset some stuff
    ...
    // stores the closest node found that has full length (i.e. its index == (N_
    // - 2) )
    ...
    std::priority_queue<Node, std::vector<Node>, CompareCost> openList_tmp(
        comparer);
    ...
    while (openList_.size() > 0) {
        ...
        *current_ptr =
            openList_.top();  // copy the first element onto (*current_ptr)
        ...
        // }
        // already_exist = false;
        if (already_exist || (*current_ptr).qi.x() > x_max_ ||
            (*current_ptr).qi.x() < x_min_ ||  /// Outside the limits
            (*current_ptr).qi.y() > y_max_ ||
            (*current_ptr).qi.y() < y_min_ ||  /// Outside the limits
            (*current_ptr).qi.z() > z_max_ ||
            (*current_ptr).qi.z() < z_min_ ||  /// Outside the limits
        ...
        if (i <
            (N_ - 2)) {  // for qNm2 I'm not going to sample velocities (not
                         // going to call expandAndAddToQueue) --> don't do this
            intervalIsNotZero = computeUpperAndLowerConstraints(
                i, qiM2, qiM1, (*current_ptr).qi, constraint_xL, constraint_xU,
                constraint_yL, constraint_yU, constraint_zL, constraint_zU);
        }
        if (intervalIsNotZero ==
            false)  // constraintxL>constraint_xU (or with other axes)
        {
            continue;
        }
        ...
        // std::cout << "collision check took " << timer_collision_check <<
        // std::endl;
        ...
        if (collides) {
            ...
            // while (tmp != NULL)
            // {
            //   // cps.push_back(Eigen::Vector3d(tmp->qi.x(), tmp->qi.y(),
            //   tmp->qi.z())); unsigned int ix2, iy2, iz2; ix2 =
            //   round((tmp->qi.x() - orig_.x()) / voxel_size_); iy2 =
            //   round((tmp->qi.y() - orig_.y()) / voxel_size_); iz2 =
            //   round((tmp->qi.z() - orig_.z()) / voxel_size_);
            ...
            // std::cout << green << bold << "does not collide: " <<
            // (*current_ptr).qi.transpose() << "(i= " << i << ")" << reset
            //           << std::endl;

            // std::cout << "pushing, index= " << (*current_ptr).index <<
            // std::endl;
            map_open_list_[Eigen::Vector3i(ix, iy, iz)] = true;
        ...
        if ((*current_ptr).index == (N_ - 2) &&
            dist <
                std::max(0.0,
                         complete_closest_dist_so_far_ -
                             1e-4))  // the 1e-4 is to avoid numerical issues of
                                     // paths essentially with the same dist to
                                     // the goal. In those cases, this gives
                                     // priority to the trajectories found first
        {
            complete_closest_dist_so_far_ = dist;
            complete_closest_result_so_far_ptr_ = current_ptr;

            // std::cout << bold << blue << "complete_closest_dist_so_far_= " <<
            // std::setprecision(10)
        ...
        expandAndAddToQueue(*current_ptr, constraint_xL, constraint_xU,
                            constraint_yL, constraint_yU, constraint_zL,
                            constraint_zU);
    }
...
exitloop:
    ...
    // std::cout << "expanded_nodes_.size()= " << expanded_nodes_.size() <<
    // std::endl; std::cout << "complete_closest_dist_so_far_= " <<
    // complete_closest_dist_so_far_ << std::endl;

    Node* best_node_ptr = NULL;

    bool have_a_solution = (complete_closest_result_so_far_ptr_ != NULL) ||
                           (closest_result_so_far_ptr_ != NULL);
    ...
    if (status == GOAL_REACHED) {
        ...
        // {
        //   std::cout << "THERE EXISTS AT LEAST ONE COMPLETE PATH, with
        //   distance= " << complete_closest_dist_so_far_
        //             << std::endl;
        // }

        if (complete_closest_result_so_far_ptr_ != NULL) {
            std::cout << "[A*] choosing closest complete path as solution"
                      << std::endl;
        ...
        std::cout << red << "This state should never occur" << reset
                  << std::endl;
        abort();
        return false;
    }
    ...
    // Note that, by doing this, it's not guaranteed feasibility wrt a dynamic
    // obstacle and hence the need of the function checkFeasAndFillND()
    ...
    if (isFeasible == false && path_found_is_not_complete == false) {
        // std::cout << red << "This should never happen: All complete paths are
        // guaranteed to be feasible" << reset
        //           << std::endl;
    ...
    if (!isFeasible) {
        std::cout << "trailing feasibility check failed!!!!!!!!" << std::endl;
    }
