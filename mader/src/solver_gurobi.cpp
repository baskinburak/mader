...
SolverGurobi::SolverGurobi(ms::par_solver &par) {
    // std::cout << "SolverGurobi constructor start" << std::endl;
    ...
    // std::cout << "before basis converter construction" << std::endl;
    ...
    if (par_.basis == "MINVO") {
        basis_ = MINVO;
        M_pos_bs2basis_ =
            basis_converter.getMinvoPosConverters(num_of_segments_);
        M_vel_bs2basis_ =
            basis_converter.getMinvoVelConverters(num_of_segments_);
    } else if (par_.basis == "BEZIER") {
        basis_ = BEZIER;
        M_pos_bs2basis_ =
            basis_converter.getBezierPosConverters(num_of_segments_);
        M_vel_bs2basis_ =
            basis_converter.getBezierVelConverters(num_of_segments_);
    } else if (par_.basis == "B_SPLINE") {
        basis_ = B_SPLINE;
        M_pos_bs2basis_ =
            basis_converter.getBSplinePosConverters(num_of_segments_);
        M_vel_bs2basis_ =
            basis_converter.getBSplineVelConverters(num_of_segments_);
    } else {
        std::cout << red << "Basis " << par_.basis << " not implemented yet"
                  << reset << std::endl;
        std::cout << red
                  << "============================================" << reset
                  << std::endl;
    ...
    // std::cout << "before basis converter .getABSpline" << std::endl;

    A_pos_bs_ = basis_converter.getABSpline(num_of_segments_);

    separator_solver_ = new separator::Separator();

    // std::cout << "before octopus solver" << std::endl;

    octopusSolver_ = new OctopusSearch(par_.basis, par_.num_pol, par_.deg_pol,
                                       par_.alpha_shrink);

    // std::cout << "after octopus solver" << std::endl;
}
...
void SolverGurobi::addObjective() {
    ...
    for (int i = 0; i < num_of_segments_; i++) {
        Eigen::Matrix<double, -1, 1> A_i_times_tmp =
            A_pos_bs_[i] * tmp;  // TODO (this is 4x1)

        GRBMatrix Q;
        Q.push_back(GRBVector{q_exp_[i][0], q_exp_[i + 1][0], q_exp_[i + 2][0],
                              q_exp_[i + 3][0]});  // row0
        Q.push_back(GRBVector{q_exp_[i][1], q_exp_[i + 1][1], q_exp_[i + 2][1],
                              q_exp_[i + 3][1]});  // row1
        Q.push_back(GRBVector{q_exp_[i][2], q_exp_[i + 1][2], q_exp_[i + 2][2],
                              q_exp_[i + 3][2]});  // row2

        control_cost_ +=
            getNorm2(matrixMultiply(Q, eigenVector2std(A_i_times_tmp)));
    }
...
void SolverGurobi::addConstraints() {
    ...
    // Final velocity and acceleration are zero \equiv
    // q_exp_[N_]=q_exp_[N_-1]=q_exp_[N_-2]
    ...
    for (int i = 0; i <= (N_ - 3); i++)  // i  is the interval (\equiv segment)
    {
        ...
        Qbs.push_back(GRBVector{q_exp_[i][0], q_exp_[i + 1][0],
                                q_exp_[i + 2][0], q_exp_[i + 3][0]});  // row0
        Qbs.push_back(GRBVector{q_exp_[i][1], q_exp_[i + 1][1],
                                q_exp_[i + 2][1], q_exp_[i + 3][1]});  // row1
        Qbs.push_back(GRBVector{q_exp_[i][2], q_exp_[i + 1][2],
                                q_exp_[i + 2][2], q_exp_[i + 3][2]});  // row2

        transformPosBSpline2otherBasis(
            Qbs, Qmv,
            i);  // each row of Qmv will contain a "basis_" control point
        ...
        for (int obst_index = 0; obst_index < num_of_obst_; obst_index++) {
            ...
            // impose that all the vertexes of the obstacle are on one side of
            // the plane for (int j = 0; j < hulls_[obst_index][i].cols(); j++)
            // // Eigen::Vector3d vertex : hulls_[obst_index][i]
            // {
            //   Eigen::Vector3d vertex = hulls_[obst_index][i].col(j);
            //   m_.addConstr((-(n_[ip].dot(vertex) + d_[ip] - epsilon)) <= 0,
            //   "plane" + std::to_string(j));
            //   // constraints[r] = -(n[ip].dot(vertex) + d[ip] - epsilon);  //
            //   f<=0
            // }
            ...
            for (int u = 0; u <= 3; u++) {
                ...
                GRBLinExpr dot = n_[ip].x() * q_ipu[0] + n_[ip].y() * q_ipu[1] +
                                 n_[ip].z() * q_ipu[2];
    ...
    for (int i = 2; i <= (N_ - 2);
         i++)  // If using BSpline basis, v0 and v1 are already determined by
               // initial_state
    {
        ...
        for (int j = 0; j < 3;
             j++) {  // loop over each of the velocity control points
                     // (v_{i-2+j}) of the new basis
    ...
    for (int i = 1; i <= (N_ - 3);
         i++)  // a0 is already determined by the initial state
    {
...
int SolverGurobi::getNumOfLPsRun() { return octopusSolver_->getNumOfLPsRun(); }

int SolverGurobi::getNumOfQCQPsRun() { return num_of_QCQPs_run_; }

void SolverGurobi::setMaxRuntimeKappaAndMu(double max_runtime, double kappa,
                                           double mu) {
    kappa_ = kappa;
    mu_ = mu;
...
void SolverGurobi::fillPlanesFromNDQ(const std::vector<Eigen::Vector3d> &n,
    ...
    for (int obst_index = 0; obst_index < num_of_obst_; obst_index++) {
        for (int i = 0; i < num_of_segments_; i++) {
            ...
            Eigen::Matrix<double, 3, 4> Qmv,
                Qbs;  // minvo. each column contains a MINVO control point
            Qbs.col(0) = q[i];
            Qbs.col(1) = q[i + 1];
            Qbs.col(2) = q[i + 2];
            Qbs.col(3) = q[i + 3];
            ...
            /////////////////// OPTION 1: point_in_plane = intersection between
            /// line  centroid_cps --> centroid_hull
            // bool intersects = getIntersectionWithPlane(centroid_cps,
            // centroid_hull, Eigen::Vector4d(A, B, C, D),
            //                                            point_in_plane);  //
            //                                            result saved in
            //                                            point_in_plane
            ...
            /////////////////// OPTION 2: point_in_plane = intersection between
            /// line  centroid_cps --> closest_vertex
            double dist_min = std::numeric_limits<double>::
                max();  // delta_min will contain the minimum distance between
            ...
            Eigen::Vector3d closest_vertex =
                hulls_[obst_index][i].col(index_closest_vertex);

            bool intersects = getIntersectionWithPlane(
                centroid_cps, closest_vertex, Eigen::Vector4d(A, B, C, D),
                point_in_plane);  // result saved in point_in_plane

            //////////////////////////

            if (intersects == false) {
                // TODO: this msg is printed sometimes in Multi-Agent
                // simulations. Find out why
                std::cout << red
                          << "There is no intersection, this should never "
                             "happen (TODO)"
                          << reset << std::endl;
                continue;  // abort();
            }
...
// returns 1 if there is an intersection between the segment P1-P2 and the plane
// given by coeff=[A B C D] (Ax+By+Cz+D==0)  returns 0 if there is no
// intersection. The intersection point is saved in "intersection"
bool SolverGurobi::getIntersectionWithPlane(const Eigen::Vector3d &P1,
    ...
    bool result = (t < 0 || t > 1)
                      ? false
                      : true;  // False if the intersection is with the line
                               // P1-P2, not with the segment P1-P2

    return result;
}

// Intersection between the ray p0<-->p1 and the plane n'x+f==0
// Eigen::Vector3d SolverGurobi::intersectPoint(Eigen::Vector3d p0,
// Eigen::Vector3d p1, Eigen::Vector3d n, double f)
// {
...
//   return p0 - rayVector * (diff.dot(planeNormal)) /
//   (rayVector.dot(planeNormal));
// }
...

{
    ...
    i_min_ = 0;
    i_max_ =
        3 * (N_ + 1) - 1 - 9 - 6;  // because pos, vel and accel at t_init_ and
                                   // t_final_ are fixed (not dec variables)
    j_min_ = i_max_ + 1;
    ...
    for (int i = 0; i <= N_; i++) {
        ...
        {
            GRBVar tmp;
            if (fabs(maxs[j] - mins[j]) >
                100) {  // this is to prevent numerical issues with the solver
                        // (happen when the bounds are too large)
                // Here we let the variable be free (by using -GRB_INFINITY and
                // GRB_INFINITY)
                tmp = m_.addVar(-GRB_INFINITY, GRB_INFINITY, 0, GRB_CONTINUOUS,
                                "q_" + std::to_string(i) + coords[j]);
            } else {
                tmp = m_.addVar(mins[j], maxs[j], 0, GRB_CONTINUOUS,
                                "q_" + std::to_string(i) + coords[j]);
            }
...
// Note that t_final will be updated in case the saturation in deltaT_ has had
// effect
bool SolverGurobi::setInitStateFinalStateInitTFinalT(mt::state initial_state,
    ...
    // the reason for this is the epsilon_tol_constraints (in the previous
    // iteration, it may be slightly unfeasible)
    ...
    //////////////////////////////
    // Now make sure deltaT in knots_ is such that -v_max<=v1<=v_max is
    // satisfied: std::cout << bold << "deltaT_ before= " << deltaT_ << reset <<
    // std::endl;
    for (int axis = 0; axis < 3; axis++) {
        double upper_bound, lower_bound;
        if (fabs(a0(axis)) > 1e-7) {
            upper_bound =
                ((p_ - 1) * (mu::sgn(a0(axis)) * par_.v_max(axis) - v0(axis)) /
                 (a0(axis)));
            lower_bound =
                ((p_ - 1) * (-mu::sgn(a0(axis)) * par_.v_max(axis) - v0(axis)) /
                 (a0(axis)));
            ...
            if (upper_bound < lower_bound) {
                std::cout << red << bold << "This should never happen, aborting"
                          << std::endl;
                abort();
            }
            //////////////////
            ...
            if (upper_bound <= 0) {
                std::cout << red << bold << "There is no way to satisfy v1"
                          << std::endl;  //(deltat will be zero)
                return false;
            }

            mu::saturate(deltaT_, std::max(0.0, lower_bound), upper_bound);
        } else {
            // do nothing: a0 ==0 for that axis, so that means that v1==v0, and
            // therefore v1 satisfies constraints for that axis
        }
    }

    // Eigen::Vector3d bound1 = ((p_ - 1) * (v_max_ - v0).array() /
    // (a0.array())); Eigen::Vector3d bound2 = ((p_ - 1) * (-v_max_ -
    // v0).array() / (a0.array()));

    // // note that if any element of a0 is ==0.0, then its corresponding
    // element in bound1 (or bound2) is +-infinity, but
    // // valid  for the saturation below

    // mu::saturate(deltaT_, std::min(bound1.x(), bound2.x()),
    // std::max(bound1.x(), bound2.x())); mu::saturate(deltaT_,
    // std::min(bound1.y(), bound2.y()), std::max(bound1.y(), bound2.y()));
    // mu::saturate(deltaT_, std::min(bound1.z(), bound2.z()),
    // std::max(bound1.z(), bound2.z()));

    // std::cout << "std::min(bound1.x(), bound2.x()= " << std::min(bound1.x(),
    // bound2.x()) << std::endl; std::cout << "std::max(bound1.x(), bound2.x()=
    // " << std::max(bound1.x(), bound2.x()) << std::endl;

    // std::cout << "std::min(bound1.y(), bound2.y()= " << std::min(bound1.y(),
    // bound2.y()) << std::endl; std::cout << "std::max(bound1.y(), bound2.y()=
    // " << std::max(bound1.y(), bound2.y()) << std::endl;

    // std::cout << "std::min(bound1.z(), bound2.z()= " << std::min(bound1.z(),
    // bound2.z()) << std::endl; std::cout << "std::max(bound1.z(), bound2.z()=
    // " << std::max(bound1.z(), bound2.z()) << std::endl;
    ...
    for (int i = (p_ + 1); i <= M_ - p_ - 1; i++) {
        knots[i] =
            knots[i - 1] + deltaT_;  // Assumming a uniform b-spline (internal
    ...
    // weight_modified_ = par_.weight * (final_state_.pos -
    // initial_state_.pos).norm();
    ...
    q2_ = (p_ * p_ * q1_ - (t1PpP1 - t2) * (a0 * (t2 - tpP1) + v0) -
           p_ * (q1_ + (-t1PpP1 + t2) * v0)) /
          ((-1 + p_) * p_);

    qN_ = pf;
    qNm1_ = pf + ((tN - tNPp) * vf) / p_;
    qNm2_ = (p_ * p_ * qNm1_ - (tNm1 - tNm1Pp) * (af * (-tN + tNm1Pp) + vf) -
             p_ * (qNm1_ + (-tNm1 + tNm1Pp) * vf)) /
            ((-1 + p_) * p_);
...
bool SolverGurobi::optimize() {
    ...
    // note that, for a v0 and a0 given, q2_ is not guaranteed to lie within the
    // bounds. If that's the case --> keep executing previous trajectory
    if (q2_.x() > par_.x_max || q2_.x() < par_.x_min ||  //////////////
        q2_.y() > par_.y_max || q2_.y() < par_.y_min ||  /////////////////
        q2_.z() > par_.z_max || q2_.z() < par_.z_min) {
        std::cout << bold << red << "q2_ is not in [min, max]" << reset
                  << std::endl;
        std::cout << "q2_= " << q2_.transpose() << std::endl;
        std::cout << "par_.x_min= " << par_.x_min
                  << ", par_.x_max=" << par_.x_max << std::endl;
        std::cout << "par_.y_min= " << par_.y_min
                  << ", par_.y_max=" << par_.y_max << std::endl;
        std::cout << "par_.z_min= " << par_.z_min
                  << ", par_.z_max=" << par_.z_max << std::endl;
        return false;
    }
    bool guess_is_feasible =
        generateAStarGuess();  // I obtain q_quess_, n_guess_, d_guess_
    ...
    // See
    // https://www.gurobi.com/documentation/9.0/refman/cpp_parameter_examples.html
    ...
    std::cout << "Starting optimization, allowing time = "
              << mu_ * max_runtime_ * 1000 << " ms" << std::endl;
    m_.optimize();
    ...
    //        optimstatus == GRB_SUBOPTIMAL //Don't include this one (sometimes
    //        it generates very long/suboptimal trajectories)
    ...
    if ((optimstatus == GRB_OPTIMAL || optimstatus == GRB_TIME_LIMIT ||
         ...
         optimstatus == GRB_ITERATION_LIMIT ||
         optimstatus == GRB_NODE_LIMIT ||  ///////////////
...

    {
        ...
        // copy the solution
        for (auto tmp : q_exp_) {
            q.push_back(Eigen::Vector3d(tmp[0].getValue(), tmp[1].getValue(),
                                        tmp[2].getValue()));
        }
        ...
        std::cout << red << "Gurobi failed to find a solution, returning"
                  << reset << std::endl;
        // q = q_guess_;

        // The guess is not guaranteed to be feasible due to jerk constraints-->
        // cannot commit to the guess --> Returning

        return false;
    }

    CPs2TrajAndPwp(q, traj_solution_, solution_, N_, p_, par_.num_pol, knots_,
                   par_.dc);
    // Force last position =final_state_ (which it's not guaranteed because of
    // the discretization with dc)
    ...
    // std::cout << blue << "traj_solution_.size()=" << traj_solution_.size()
    // <<reset<< std::endl;

    // Uncomment the following line if you wanna visualize the planes
    // fillPlanesFromNDQ(n_, d_, q);  // TODO: move this outside the
    // SolverGurobi class
