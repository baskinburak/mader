...
template <typename T>
GRBQuadExpr getNorm2(
    const std::vector<T>& x)  // Return the squared norm of a vector
{
...
inline void addVectorEqConstraint(GRBModel& m, const GRBVector a,
...
inline void addVectorLessEqualConstraint(GRBModel& m, const GRBVector a,
                                         const Eigen::Vector3d& b) {
    for (int i = 0; i < a.size(); i++) {
        m.addConstr(a[i] <= b[i]);
    }
...
inline void addVectorGreaterEqualConstraint(GRBModel& m, const GRBVector a,
                                            const Eigen::Vector3d& b) {
    for (int i = 0; i < a.size(); i++) {
        m.addConstr(a[i] >= b[i]);
    }
...
inline void resetCompleteModel(GRBModel& m) {
    ...
    m.reset();  // Note that this function, only by itself, does NOT remove vars
                // or constraints
}
...
GRBVector matrixMultiply(const std::vector<std::vector<R>>& A,
...
template <typename T>
std::vector<GRBVector> matrixMultiply(
    const std::vector<std::vector<T>>& A,
    const std::vector<std::vector<double>>& B) {
    std::vector<GRBVector> result(
        A.size(), GRBVector(B[0].size(), 0.0));  // Initialize all the
                                                 // elements to zero
...
template <typename T>
std::vector<T> eigenVector2std(
    const Eigen::Matrix<T, 3, 1>& x)  // TODO: Merge with the previous one?
{
    std::vector<T> result;
    for (int i = 0; i < x.rows(); i++) {
        result.push_back(x(i));
...
inline std::vector<std::vector<double>> eigenMatrix2std(
...
//   std::vector<double>(B[0].size(), 0.0));  // Initialize all the
//                                                                                              // elements to zero
...
// template <typename T, typename R>
// GRBVector matrixMultiply(const std::vector<std::vector<R>>& A, const
// std::vector<std::vector<R>>& B)
// {
//   std::vector<GRBVector> result;
...
// // template <typename T>
// Eigen::Matrix<GRBLinExpr, -1, -1> myMultDV(const Eigen::Matrix<double, -1,
// -1>& A,
//                                            const Eigen::Matrix<GRBVar, -1,
//                                            1>& b)
// {
...
// Eigen::Matrix<GRBLinExpr, -1, -1> myMultVD(const Eigen::Matrix<GRBVar, -1,
// -1>& A,
...
// Eigen::Matrix<GRBLinExpr, -1, -1> operator-(const Eigen::Matrix<GRBLinExpr,
// -1, -1>& a,
//                                             const Eigen::Matrix<GRBLinExpr,
//                                             -1, -1>& b)
// {
...
// Eigen::Matrix<GRBLinExpr, -1, -1> myMult(const Eigen::Matrix<GRBVar, -1, -1>&
// A, const Eigen::Matrix<GRBVar, -1, 1>& b)
// {
//   Eigen::Matrix<GRBLinExpr, -1, -1> result(A.rows(), b.cols());
...
// Eigen::Matrix<GRBLinExpr, 2, 1> operator*(const Eigen::Matrix<double, 2, 2>&
// A, const Eigen::Matrix<GRBVar, 2, 1>& b)
// {
...
// template <typename T>
// Eigen::Matrix<GRBLinExpr, 2, 1> operator*(const Eigen::MatrixBase<Derived1>&
// A, const Eigen::MatrixBase<Derived2>& b)
// {
//   Eigen::Matrix<GRBLinExpr, 2, 1> result;
...
/*GRBVector MatrixMultiply(const std::vector<std::vector<double>>& A, const
GRBVector& x)
{
  GRBVector result;
  for (int i = 0; i < A.size(); i++)
